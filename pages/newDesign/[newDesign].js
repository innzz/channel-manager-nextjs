    ...
    if (newDesign !== undefined) {
      fetch(
        ...
        .then((resJson) => {
          ...
          setShowTravelAgencyName(resJson.propertiesOnlineTravelAgencies[2]);
  ...
  useEffect(() => {
    ...
    if (showTravelAgencyName.onlineTravelAgencyName == "Agoda") {
      const data = {
        ...
        toDate: newSevenDay,
        id: showTravelAgencyName.onlineTravelAgencyPropertyId,
      };
      fetch(`https://channel-manager-server.herokuapp.com/propertyData`, {
        ...
        .then((resJson) => {
          ...
          for (
            let i = 0;
            i < resJson.result.properties[0].property.length;
            i++
          ) {
            const date = new Date(
              resJson.result.properties[0].property[i].$.date
            );
